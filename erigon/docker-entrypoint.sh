...
if [[ ! "${COMPOSE_FILE}" =~ "caplin.yml" ]]; then
  ...
  __caplin="--caplin.discovery.addr=0.0.0.0 --caplin.discovery.port=${CL_DISC_PORT} --caplin.blobs-immediate-backfill=true"
  __caplin+=" --caplin.discovery.quicport=${CL_QUIC_PORT} --caplin.validator-monitor=true"
